...
Pela lei da probabilidade total, a marginal de $X$ é $N(1,5)$, e portanto o valor exato é $\theta = P(X > 1) = 0.5$.O método de Monte Carlo simples simula diretamente pares $(Y, X)$ e calcula a proporção de vezes em que $X > 1$.

O estimador (para $\theta = \mathbb{E}[I(X>1)]$ é simples: $\hat\theta_{MC} = \frac{1}{K}\sum_{i=1}^K I(X_i > 1)$, onde $I(\cdot)$ é a função indicadora. Esse método é fácil, mas apresenta alta variância porque depende da dupla simulação de $Y$ e do erro $\varepsilon$.
...
Podemos reduzir a variância substituindo a indicadora $I(X>1)$ por sua esperança condicional dado $Y$:

$$
g(Y) = E[I(X > 1) \mid Y] = P(X > 1 \mid Y) = 1 - \Phi!\left(\frac{1 - Y}{2}\right) = \Phi!\left(\frac{Y - 1}{2}\right)
$$
Assim, o novo estimador é $\hat\theta_{EC} = \frac{1}{K} \sum_{i=1}^K g(Y_i)$. Faz sentido utilizar esse novo estimador uma vez que esse método elimina a variabilidade extra introduzida pela simulação de $X$, e como consequência, a variância de $\hat\theta_{EC}$ é sempre menor ou igual à de $\hat\theta_{MC}$ pelo teorema de Rao-Blackwell.
...
Utilizando uma variável de controle, o objetivo é reduzir ainda mais a variância usando uma variável correlacionada conhecida. Nesse caso, como $Y$ tem média conhecida ($E[Y]=1$), podemos usá-la como controle:

$$
\hat\theta_{VC} = \hat\theta_{EC} - c ( \bar Y - E[Y]),
$$

onde o coeficiente ótimo é $c^\ast = \frac{\text{Cov}(g(Y), Y)}{\text{Var}(Y)}$.

A variável de controle remove parte da variabilidade residual ainda presente em $g(Y)$: se $Y$ estiver fortemente correlacionado com $g(Y)$, a redução de variância pode ser expressiva.
...
Podemos comparar empiricamente, através das simulações, a variabilidade, a média estimada (viés) e a redução percentual de variância entre os métodos.
...
Logo, concluímos que o estimador de Monte Carlo simples é correto, mas ineficiente. A esperança condicional (de acordo com o teorema de Rao-Blackwell) melhora a eficiência sem introduzir viés e a adição da variável de controle $Y$ produz ganhos adicionais, embora menores, refinando o estimador.
...
rnormtrunc <- function(n, mu=0, sigma=1, a, b){
  ...
  p2 <- pnorm(b, mu, sigma)
  p <- p1 + (p2-p1)*us
  amostra <- qnorm( p, mean=mu, sd=sigma )
}
...
Para estimar os parâmetros de um modelo Poisson, podemos usar o **método de Newton-Raphson unidimensional**, como o apresentado em aula.  
A ideia é encontrar o valor de $\beta_0$ que **zera a derivada da função de log-verossimilhança** — ou seja, onde o **score** é igual a zero.

A distribuição dos dados é dada por:  
$$y_i \sim Poisson(\lambda_i), \quad \text{com } \lambda_i = \exp(\beta_0 + \beta_1 x_i)$$  

Queremos encontrar o $\beta_0$ que satisfaz:  
$$U(\beta_0) = \sum_i (y_i - \lambda_i) = 0$$  
onde $\lambda_i = \exp(\beta_0 + \beta_1 x_i)$.

Esse $U(\beta_0)$ é a **primeira derivada** da log-verossimilhança em relação a $\beta_0$, e é a função que passamos para o algoritmo do slide.

### Passo a passo do algoritmo

1. **Gerar os dados do modelo Poisson**  
   - Escolhemos valores de $\beta_0$ e $\beta_1$.  
   - Geramos $x_i$ (por exemplo, de uma distribuição uniforme) e depois calculamos  
     $\lambda_i = \exp(\beta_0 + \beta_1 x_i)$.  
   - Finalmente, geramos $y_i \sim Poisson(\lambda_i)$.

2. **Definir a função Score**  
   $$f(\beta_0) = \sum_i (y_i - \exp(\beta_0 + \beta_1 x_i))$$  
   Essa é a função cujas raízes queremos encontrar.

3. **Aplicar o método de Newton-Raphson (função do slide)**  
   - Começamos com um chute inicial $x_0$ (valor inicial de $\beta_0$).  
   - Em cada iteração, atualizamos:
     $$\beta_0^{(t+1)} = \beta_0^{(t)} - \frac{f(\beta_0^{(t)})}{f'(\beta_0^{(t)})}$$  
     onde $f'(\beta_0^{(t)})$ é a derivada numérica de $f$ em relação a $\beta_0$, calculada automaticamente pela função `genD()` do pacote **numDeriv**.

4.  O processo é repetido até que a diferença entre duas iterações consecutivas seja menor que a tolerância definida (`tol`).


```{r}
# Função do slide (Newton-Raphson)
newton.raphson <- function(f, a, b, tol = 1e-8, n = 1000, x0 = 0) {
  if (!requireNamespace("numDeriv", quietly = TRUE)) {
    stop("Instale o pacote 'numDeriv': install.packages('numDeriv')")
  }
  grad <- numDeriv::grad
  k <- numeric(n)
  x <- x0
  for (i in 1:n) {
    dx <- grad(f, x)
    if (abs(dx) < .Machine$double.eps) break
    x1 <- x - (f(x) / dx)
    k[i] <- x1
    if (abs(x1 - x) < tol) {
      return(list(raiz = x1, iteracoes = i))
    }
    x <- x1
  }
  warning("Sem convergência")
  list(raiz = x, iteracoes = n)
}
```


```{r}
#  Gera dados e estima beta0 fixando beta1 
estimate_beta0_fix_beta1 <- function(beta0_true, beta1_true, beta1_fixed,
                                     n = 200, x_range = c(-2, 2),
                                     a = -5, b = 5, x0 = 0) {
  x <- runif(n, min = x_range[1], max = x_range[2])
  lambda <- exp(beta0_true + beta1_true * x)
  y <- rpois(n, lambda)
  score_beta0 <- function(beta0) sum(y - exp(beta0 + beta1_fixed * x))
  res <- try(newton.raphson(f = score_beta0, a = a, b = b, x0 = x0), silent = TRUE)
  if (inherits(res, "try-error")) return(NA)
  res$raiz
}

# Simulação de vários cenários 
run_scenario <- function(name, beta0, beta1, beta1_fixed, n, reps = 200) {
  est <- replicate(reps, estimate_beta0_fix_beta1(beta0, beta1, beta1_fixed, n))
  data.frame(
    scenario = name,
    beta0_hat = est,
    beta0_true = beta0,
    beta1_true = beta1,
    beta1_fixed = beta1_fixed,
    n = n
  )
}

scenarios <- list(
  list(name = "1_intercepto_apenas", beta0 = 1.0, beta1 = 0.0, beta1_fixed = 0.0, n = 200),
  list(name = "2_beta1_correto", beta0 = 1.5, beta1 = -0.8, beta1_fixed = -0.8, n = 200),
  list(name = "3_beta1_errado", beta0 = 1.5, beta1 = -0.8, beta1_fixed = -0.4, n = 200),
  list(name = "4_maior_n", beta0 = 1.5, beta1 = -0.8, beta1_fixed = -0.8, n = 2000)
)

library(dplyr)
results <- bind_rows(lapply(scenarios, function(s) {
  run_scenario(s$name, s$beta0, s$beta1, s$beta1_fixed, s$n)
}))
...
summary_table <- results %>%
  group_by(scenario) %>%
  summarise(
    beta0_true = first(beta0_true),
    mean_est = mean(beta0_hat, na.rm = TRUE),
    sd_est = sd(beta0_hat, na.rm = TRUE),
    bias = mean(beta0_hat - beta0_true, na.rm = TRUE)
  )
print(summary_table)
```
...
### Algoritmo EM dos slides:

```{r Q5_1}
set.seed(156)

# Gerando dados com excesso de zero 
rPoisson_zero <- function(n, lambda, p){
  s <- rbinom(n, 1, p) # valor 1 indica um zero em excesso
  x <- rpois(n, lambda) * (s == 0) 
}

## Algoritmo EM - Poisson zero inflacionado
EM_Poisson_zinf <- function(x, M){
  # 1. Inicialize p^(0) e lambda^(0)
  lambda <- 5
  p <- 0.05
  ...
  # 3. Repita os passos abaixo até atingir a convergência:
  
  while(t < M){
    
    ##  Calcule gamma_{i1}(t) e gamma_{i0}(t)
    aux1 <- p * (x == 0)*1 # vetor de tamanho n
    aux2 <- (1-p) * dpois(x, lambda) # vetor de tamanho n
    gamma1 <- aux1 / (aux1 + aux2) # vetor de tamanho n relativo a excessos de zero
    gamma0 <- 1 - gamma1
    
    ##  Obtenha p^(t+1) e lambda^(t+1) com os estimadores obtidos no passo de maximização; 
    n <- length(x)
    p <- sum(gamma1)/n
    lambda <- sum(x*gamma0)/sum(gamma0)
  ...
  return(list(p = p_vec, lambda = lambda_vec))
}
```
...
# Parâmetros fixos
lambda_real <- 3
p_valores <- c(0, 0.05, 0.10, 0.20)
n <- 500          # tamanho amostral
M_iter <- 100     # iterações do EM
R <- 100          # número de réplicas Monte Carlo

# Armazenar resultados
resultados <- data.frame(
  p_verdadeiro = rep(p_valores, each = R),
  p_estimado = NA,
  lambda_estimado = NA
)
...
i <- 1
for(p_verdadeiro in p_valores){
  
  for(r in 1:R){
    # Gerar dados
    x <- rPoisson_zero(n, lambda_real, p_verdadeiro)
    
    # Rodar EM
    ajuste <- EM_Poisson_zinf(x, M_iter)
    
    # Guardar estimativas finais (último valor)
    resultados$p_estimado[i] <- tail(ajuste$p, 1)
    resultados$lambda_estimado[i] <- tail(ajuste$lambda, 1)
...
```{r Q5_3, echo=FALSE}
# Médias e desvios-padrão por cenário
resumo <- aggregate(
  cbind(p_estimado, lambda_estimado) ~ p_verdadeiro,
  data = resultados,
  FUN = function(x) c(media = mean(x), desvio = sd(x))
)

resumo_formatado <- data.frame(
  p_verdadeiro = resumo$p_verdadeiro,
  media_p = round(resumo$p_estimado[, "media"], 3),
  desvio_p = round(resumo$p_estimado[, "desvio"], 3),
  media_lambda = round(resumo$lambda_estimado[, "media"], 3),
  desvio_lambda = round(resumo$lambda_estimado[, "desvio"], 3)
)
...
# Boxplot das estimativas de p
ggplot(resultados, aes(x = factor(p_verdadeiro), y = p_estimado)) +
  geom_boxplot(fill = "#56B4E9", color = "black", alpha = 0.7) +
  geom_hline(aes(yintercept = p_verdadeiro), color = "red", linetype = "dashed") +
  labs(
    title = "Distribuição das estimativas de p (100 réplicas)",
    x = "p verdadeiro",
    y = "p estimado"
  ) +
  theme_minimal(base_size = 13)

# Boxplot das estimativas de lambda
ggplot(resultados, aes(x = factor(p_verdadeiro), y = lambda_estimado)) +
  geom_boxplot(fill = "#009E73", color = "black", alpha = 0.7) +
  geom_hline(yintercept = lambda_real, color = "red", linetype = "dashed") +
  labs(
    title = "Distribuição das estimativas de λ (100 réplicas)",
    x = "p verdadeiro",
    y = "λ estimado"
  ) +
  theme_minimal(base_size = 13)
...
Os resultados mostram que o algoritmo EM recupera bem os parâmetros verdadeiros em todos os cenários considerados.
