...
author: "Emanuelle Oliveira, Mariana Fleming, Maria Luiza Oliveira"
date: "2025-10-02"
...
hist(amostra, 
     ...
     xlab = "Valores da Amostra (Qui-Quadrado)",
     ylab = "Densidade",
     col = "lightblue",
     border = "gray40",
     xlim = c(0, 8),
...
## SIR Slash
```{r sir_slash}
slash_sir_t <- function(n, n_cand = n * 20, df_proposta = 2){
  
  # 1. Gerar candidatos da proposta t de Student
  x_cand <- rt(n_cand, df = df_proposta)
  
  # 2. Calcular a densidade alvo (f) numericamente
...
slash_sir_t <- function(n, n_cand = n * 20, df_proposta = 1){
  x_cand <- rt(n_cand, df = df_proposta)
...
amostra_gerada <- slash_sir_t(n = n_amostra, n_cand = 100000, df_proposta = 4)
...
mtext("Método SIR com proposta t-Student com 5 g.l.", side = 1, line = 4, cex = 0.9, font = 3)

grid <- seq(-10, 10, length.out = 500)
densidade_teorica <- dslash(grid)
lines(grid, densidade_teorica, 
      col = "red",
      lwd = 2)

legend("topright", 
       legend = c("Histograma da Amostra", "Densidade Teórica (dslash)"),
       pch = c(22, NA),
       pt.bg = c("lightblue", NA),
       pt.cex = 1.6,
       col = c("gray40", "red"),
...
geradoras_list <- list(
  ...
  "Slash"               = function(n) slash_sir_t(n = n) 
)
...
grafico_poder <- ggplot(results_long, 
  ...
  scale_color_manual(values = c(
    "Anderson-Darling" = "red3",
    "Jarque-Bera" = "darkblue",
    "Shapiro-Wilk" = "palegreen3"
  )) +
